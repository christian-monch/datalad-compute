...
**This code is a POC, that means: ncode does not
thoroughly validate inputs, names might be inconsistent, little docs.
Tests do mostly not exist.**
...
URL-KEY with an associated URL that describes the computation that should be performed (a
"computation description URL"). This
...
`ls -l text-2.txt` will show a link to a not-downloaded URL-KEY.
`git annex whereis text-2.txt` will show the associated computation description URL.
